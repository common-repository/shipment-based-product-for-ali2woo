...
if ( ! class_exists( 'WC_Ali_Front' ) ) {
    ...
    class WC_Ali_Front {
        ...
        public function single_product_custom_button() {
            ...
            wc_get_template(
                    'single-product/add-to-cart/simple.php', array(), '', WC_Ali_Products_Shipment()->plugin_path() . '/templates/'
            );
        }

        public function variable_product_custom_button() {
            wc_get_template(
                    'single-product/add-to-cart/variable.php', array(), '', WC_Ali_Products_Shipment()->plugin_path() . '/templates/'
            );
        }

        public function grouped_product_custom_button() {
            wc_get_template(
                    'single-product/add-to-cart/grouped.php', array(), '', WC_Ali_Products_Shipment()->plugin_path() . '/templates/'
            );
        }

        public function external_product_custom_button() {
            wc_get_template(
                    'single-product/add-to-cart/external.php', array(), '', WC_Ali_Products_Shipment()->plugin_path() . '/templates/'
            );
        }
